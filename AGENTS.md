...
- Live Azure read-back currently proves that the intended `$10` resource-group budget has no notification, threshold, or Action Group connection. The deallocate-only Logic App and Action Group exist, but they are not an active budget backstop. Do not claim otherwise until strict read-back shows threshold `50` and one exact Action Group.
...
- The compute target remains `Standard_B2as_v2`. The owner approved automated Monday-Friday availability from 10:00 through 18:00 Bangladesh time, implemented as a 09:50 start request and an independent 18:00 deallocation. Outside that window the normal state is `VM deallocated`; public visitors never receive start authority. The earlier two-hour owner-started workflow remains a private weekend/manual override.
- Scheduled start and stop use separate system-assigned identities and exact-VM roles. The starter cannot stop or deallocate; the stopper cannot start. Both schedules require strict resource, recurrence, identity, role, and assignment read-back before configuration is reported successful.
- Commit `6c7b06a` contains the weekday scheduler, but it has not yet been activated or observed in Azure. Until then, the private two-hour session remains the proven runtime path.
