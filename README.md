...
The cost-controlled deployment uses a burstable `Standard_B2as_v2` with bounded container logs, 6.66 GiB of aggregate memory ceilings, one loaded Ollama model, and a proven private two-hour session. PostgreSQL/pgvector, Redis, RabbitMQ, Ollama, Sentinel, Caddy, the 64-GiB disk, static IP, stable DNS name, and named data volumes remain intact. A least-privilege Monday-Friday 10:00-18:00 Bangladesh-time scheduler is committed, but live Azure activation and first-run evidence are still pending; until then the private session is the working start path.
...
At July 2026 USD retail rates, the former always-on B4as-v2 design modeled approximately `$2.6576/day`. A normally deallocated B2as-v2 deployment with one two-hour session models approximately `$0.3944/day`. The approved scheduled window starts at 09:50 for boot lead and deallocates at 18:00 on weekdays; it models approximately `$0.6978` per active weekday or `$17.72` in a 22-weekday month. Actual agreements, taxes, month length, Logic App executions, and later pricing can differ.

A read-only GitHub workflow checks nightly at 20:30 UTC that the VM is still `Standard_B2as_v2` and `VM deallocated`. It reports drift but cannot start, stop, resize, or deploy the VM. The owner-session Logic App enforces the two-hour lease. A separate deallocate-only budget workflow and Action Group exist, but live budget read-back still shows no notification connection, so they are not currently an active emergency backstop.

Azure budget notifications do not create a mathematically exact spending cap because cost records are delayed. The private wake callback is an owner credential and is never exposed to public visitors; otherwise bots could directly spend the subscription. While deallocated, the résumé URL remains identical but is offline, and disk/IP charges continue. Deployment, runtime scheduling, and budget-alert wiring are separate control planes and are verified independently.
