...
az vm create --resource-group "$resource_group" --name "$vm_name" --location "$location" --nics sentinel-demo-nic --image Ubuntu2404 --size Standard_B2as_v2 --admin-username azureuser --generate-ssh-keys --os-disk-size-gb 64 --storage-sku StandardSSD_LRS --custom-data deployment/azure-demo/bootstrap-ubuntu.sh --output none
