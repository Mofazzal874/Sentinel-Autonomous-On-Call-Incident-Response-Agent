...
public interface RunbookRepository extends JpaRepository<Runbook, UUID> {
    ...
    Optional<Runbook> findByTitle(String title);

    @Query("""
            select runbook
            from Runbook runbook
            where runbook.archivedAt is null
              and (lower(runbook.title) like lower(concat('%', :symptom, '%'))
    ...
    Page<Runbook> findByArchivedAtIsNull(Pageable pageable);
}
